...
function renderPartyEditor(date, time) {
  ...
  // 8. 파티 저장 및 디스코드 전송 기능 연동 (통합 헬퍼)
  const executePartySave = async (isDiscordSend, btnEl) => {
    const partyData = [];
    ...
    btnEl.disabled = true;
    btnEl.innerHTML = `<span class="spinner-icon"></span> 처리 중...`;
    ...
    const res = await callApi({
      ...
      sendDiscord: isDiscordSend ? 'true' : 'false'
    });
  ...
  // 두 버튼에 각각 다른 플래그를 달아 이벤트 연결
  document.getElementById('savePartyBtn').onclick = () => executePartySave(false, document.getElementById('savePartyBtn'));
  document.getElementById('sendDiscordBtn').onclick = () => executePartySave(true, document.getElementById('sendDiscordBtn'));
